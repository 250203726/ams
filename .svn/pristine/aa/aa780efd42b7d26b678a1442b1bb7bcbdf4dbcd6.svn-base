...
public class RadioboxTag extends BodyTagSupport{
	...
	private static final long serialVersionUID = -4573672843267195001L;
	...
	public int doEndTag() throws JspException {
		if(log.isInfoEnabled()) log.info("正在执行生成单选按钮标签操作...");
		try{
			//取出该组的所有数据
			if(!StringUtils.isEmpty(groupName)){
				...
				if(!dicList.isEmpty()){
					...
					for(Dictionary dic:dicList){
						...
						pageContext.getOut().println("<input type='radio' id='"+name+"_"+dic.getValue()
			...
			if (log.isInfoEnabled())
				log.error("根据<groupName=" + groupName + ">生成单选按钮失败，原因是："
						+ e.getMessage());
			e.printStackTrace();
		}
		return EVAL_PAGE;
