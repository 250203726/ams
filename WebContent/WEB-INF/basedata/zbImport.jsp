        ...
        var fileName="";
     	$(document).ready(function(){
	     		...
	     		$("#file").uploadify({
	     			...
	     			onUploadSuccess : function(file, data, response){  //上传一次完成
	     				if(""!=data && data.length>0){
		     				fileName=data;
		     				parent.Public.ajaxPost("${path}/import/zbView",{path:fileName},initData);
	     				}else{
     		...
     		$("#loadData").delegate("#btnSave","click",function(){
     			parent.Public.ajaxPost("${path}/import/zbImport",{path:fileName},save);
     		});
     	});
     	...
     	function initData(data){
     		if(data.state){
         		...
         		for(var i=0;i<data.rows.length;i++){
         			...
         			_html+='	<td class="no_wrap">'+data.rows[i].moduleType.replace(".0","")+'</td>';
...
<body>
	<div class="man_zone">
		<div class="fileList" style="padding:10px 10px">
			...
			<p>1.excel列名：合同编码、工程名称、模块类型、合同状态、合同性质、工程类型、合同金额、签订时间、签订方(甲方)、电压等级、报装容量、联系人、联系电话、备注，顺序不可变，带‘*’不能为空；</p>
			<p>2.该为文本类型且用数字表示，模块类型：
				<c:forEach items="${moduleList }" var="item" varStatus="sta">
			...
			<p>4.4.总包编码规则：SS-XX XXXX XX XXX，如SS-GJ201401001，第一部分是公建或住建简码，第二部分是年份，第三部分是月份，第四部分是流水号；</p>
